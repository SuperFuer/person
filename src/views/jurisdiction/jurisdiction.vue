...
<!--               <el-button size="mini" @click.native="editUserForm(scope.row.id, scope.row.usrId, scope.row.name, scope.row.loginName, scope.row.mail, scope.row.mobile, scope.row.deptName, scope.row.idNum)">修改</el-button>
              ...
              <el-button size="mini" @click.native="resetPassForm(scope.row.id)" type="success" plain>重置密码</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分配角色-弹窗-->
        ...
        <!-- 用户修改 -->
        <edit-userfrom :dialogEditUser.sync="editVisible"
        :userId="userId"
        :usrId="usrId"
        :name="name"
        :loginName="loginName"
...
export default {
  ...
  methods: {
    ...
    userGetList (current, size) {
      ...
      axiosGet('base/user/list?current=' + current + '&size=' + size).then(result => {
        if (result.code === 200) {
    ...
    // 搜索
    onSubmit() {
      axiosGet('base/user/list?loginName=' + this.formInline.loginName + '&name=' + this.formInline.name + '&mobile=' + this.formInline.mobile).then(result => {
        if (result.code === 200) {
          this.tableData = result.data.records
          this.totalCount = result.data.total
        } else {
    ...
    ExportData () {
      import('@/vendor/Export2Excel').then(excel => {
        // 表格的表头列表
        const tHeader = [ '部门名称', '身份证号', '姓名', '登录名', '用户职称', '邮箱', '电话', '状态']
        // 与表头相对应的数据里边的字段
        const filterVal = ['deptName', 'idNum', 'name', 'loginName', 'title', 'mail', 'mobile', 'status' ]
    ...
    // 修改
    editUserForm: function (id, usrId, name, loginName, mail, mobile, deptName, idNum) {
      this.userId = id
      this.usrId = usrId
      this.name = name
