...
<template>
	<div class="jsadmin">
		...
		<!-- 表格 -->
		<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" border style="width: 99.9%;" :header-cell-style="{
                  'font-size':'14px',
                  'padding': '8px 0',
		 ...
		 :cell-style="{
          ...
          sortable>
			...
			<el-table-column prop="EnlistedDate" label="入伍年月" width="120" sortable>
				<template slot-scope="scope">
					<span>{{scope.row.EnlistedDate?scope.row.EnlistedDate.split('T')[0]:""}}</span>
				</template>
			</el-table-column>
			<el-table-column prop="PoliticalFace" label="政治面貌" width="120" sortable>
			</el-table-column>
			<el-table-column prop="Education" label="文化程度" width="120" sortable>
			</el-table-column>
		...
		<el-dialog top="5vh" width="50%" title="权限配置" :visible.sync="dialogVisible" :before-close="hidePanel" class="jur-dialog">
			<el-tree :data="department" ref="tree" show-checkbox node-key="id" :props="defaultProps">
			</el-tree>
